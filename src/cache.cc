...
void CACHE::handle_fill()
{
  ...
  if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        (way == LLC_WAY))  // this is a bypass that does not fill the LLC
    {
      ...
      if (cache_type == IS_LLC) {
        llc_update_replacement_state(fill_cpu,
                                     set,
                                     way,
                                     MSHR.entry[mshr_index].full_addr,
                                     MSHR.entry[mshr_index].ip,
        ...
        update_replacement_state(fill_cpu,
                                 set,
                                 way,
                                 MSHR.entry[mshr_index].full_addr,
                                 MSHR.entry[mshr_index].ip,
    ...
    if (do_fill) {
      ...
      if (cache_type == IS_L1D) {
        l1d_prefetcher_cache_fill(MSHR.entry[mshr_index].full_addr,
                                  set,
                                  way,
...
void CACHE::handle_writeback()
{
  ...
  if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) &&
    ...
    if (way >= 0) {  // writeback hit (or RFO hit for L1D)
      ...
      if (cache_type == IS_LLC) {
        llc_update_replacement_state(writeback_cpu,
                                     set,
                                     way,
                                     block[set][way].full_addr,
                                     WQ.entry[index].ip,
        ...
        update_replacement_state(writeback_cpu,
                                 set,
                                 way,
                                 block[set][way].full_addr,
                                 WQ.entry[index].ip,
      ...
      if (cache_type == IS_L1D) {  // RFO miss
        ...
        if (cache_type == IS_LLC) {
          way = llc_find_victim(writeback_cpu,
        ...
        if (do_fill) {
          // update prefetcher
          if (cache_type == IS_L1D)
            l1d_prefetcher_cache_fill(WQ.entry[index].full_addr,
                                      set,
                                      way,
                                      0,
  ...
  for (uint32_t i = 0; i < MAX_READ; i++) {
    ...
    if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) &&
      ...
      if (way >= 0)  // read hit
      {
        ...
        // update replacement policy
        if (cache_type == IS_LLC)
          llc_update_replacement_state(read_cpu,
                                       set,
                                       way,
                                       block[set][way].full_addr,
                                       RQ.entry[index].ip,
          ...
          update_replacement_state(read_cpu,
                                   set,
                                   way,
                                   block[set][way].full_addr,
                                   RQ.entry[index].ip,
      ...
      } else  // read miss
      {
            ...
            (MSHR.occupancy < MSHR_SIZE))  // this is a new miss
        {
          if (cache_type == IS_LLC) {
            ...
            if (lower_level)  // add it to the next level's read queue
            {
              lower_level->add_rq(&RQ.entry[index]);
              ...
              if (RQ.entry[index].instruction) {
                ...
                if (RQ.entry[index].instr_merged) {
                  MSHR.entry[mshr_index].rob_index_depend_on_me.join(
                    RQ.entry[index].rob_index_depend_on_me,
                    ROB_SIZE);
                  DP(if (warmup_complete[MSHR.entry[mshr_index].cpu]) {
                    cout << "[INSTR_MERGED] " << __func__
                         << " cpu: " << MSHR.entry[mshr_index].cpu
  ...
  for (uint32_t i = 0; i < MAX_READ; i++) {
    ...
    if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) &&
      ...
      if (way >= 0)  // prefetch hit
      {
        // update replacement policy
        if (cache_type == IS_LLC) {
          llc_update_replacement_state(prefetch_cpu,
                                       set,
                                       way,
                                       block[set][way].full_addr,
                                       PQ.entry[index].ip,
          ...
          update_replacement_state(prefetch_cpu,
                                   set,
                                   way,
                                   block[set][way].full_addr,
                                   PQ.entry[index].ip,
        ...
        if (cache_type == IS_L1D) {
          l1d_prefetcher_prefetch_hit(block[set][way].address
                                        << LOG2_BLOCK_SIZE,
                                      PQ.entry[index].ip,
                                      PQ.entry[index].pf_metadata);
        } else if (cache_type == IS_L2C) {
          l2c_prefetcher_prefetch_hit(block[set][way].address
                                        << LOG2_BLOCK_SIZE,
                                      PQ.entry[index].ip,
                                      PQ.entry[index].pf_metadata);
        }
        if (cache_type == IS_LLC) {
          llc_prefetcher_prefetch_hit(block[set][way].address
                                        << LOG2_BLOCK_SIZE,
                                      PQ.entry[index].ip,
                                      PQ.entry[index].pf_metadata);
        }
          ...
          if (lower_level) {
            if (cache_type == IS_LLC) {
              ...
              if (lower_level->get_occupancy(3, PQ.entry[index].address) ==
                ...
                lower_level->add_pq(&PQ.entry[index]);  // add it to the lower
                                                        // level prefetch queue
              }
            }
          }
        } else {
...
int CACHE::kpc_prefetch_line(uint64_t base_addr,
                             uint64_t pf_addr,
                             int      pf_fill_level,
                             int      delta,
                             int      depth,
...
void CACHE::broadcast_bw(uint8_t bw_level)
{
  ...
  for (uint32_t core = 0; core < NUM_CPUS; ++core) {
    ...
    if (upper_level_icache[core] &&
        upper_level_icache[core] != upper_level_dcache[core]) {
...
void CACHE::prefetcher_feedback(uint64_t &pref_gen,
                                uint64_t &pref_fill,
                                uint64_t &pref_used,
                                uint64_t &pref_late)
{
...
{
  bool found = false;
  auto it = find_if(page_buffer.begin(), page_buffer.end(), [page](uint64_t p) {
    return p == page;
  });
...
void CACHE::handle_prefetch_feedback()
{
  ...
  if (knob::measure_cache_acc && cycle >= next_measure_cycle) {
    this_epoch_accuracy = pf_filled_epoch
                            ? 100 * (float)pf_useful_epoch / pf_filled_epoch
                            : 0;
    pref_acc  = (pref_acc + this_epoch_accuracy) / 2;  // have some hysterisis
    acc_level = (pref_acc /
                 ((float)100 / CACHE_ACC_LEVELS));  // quantize into 8 buckets
