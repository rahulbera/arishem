      ...
      if (!fread(&current_cloudsuite_instr, instr_size, 1, trace_file)) {
        ...
        if (ROB.occupancy < ROB.SIZE) {
          ...
          if (arch_instr.is_branch) {
            ...
            // note that branch predictor is still learning with real branch
            results if (all_warmup_complete == 0) branch_prediction =
            arch_instr.branch_taken; else branch_prediction =
            predict_branch(arch_instr.ip);
            */
            ...
            if (arch_instr.branch_taken != branch_prediction) {
              ...
              DP(if (warmup_complete[cpu]) {
                cout << "[BRANCH] MISPREDICTED instr_id: " << instr_unique_id
                     << " ip: " << hex << arch_instr.ip << dec;
                cout << " taken: " << +arch_instr.branch_taken
                     << " predicted: " << +branch_prediction << endl;
              });
              ...
              if (branch_prediction == 1) {
                // if we are accurately predicting a branch to be taken, then we
                // can't possibly fetch down that path this cycle, so we have to
                // wait until the next cycle to fetch those
                instrs_to_read_this_cycle = 0;
              }

              DP(if (warmup_complete[cpu]) {
                cout << "[BRANCH] PREDICTED    instr_id: " << instr_unique_id
                     << " ip: " << hex << arch_instr.ip << dec;
                cout << " taken: " << +arch_instr.branch_taken
                     << " predicted: " << +branch_prediction << endl;
              });
...
void O3_CPU::fetch_instruction()
{
  ...
  uint32_t read_index = (ROB.last_read == (ROB.SIZE - 1)) ? 0
                                                          : (ROB.last_read + 1);
  for (uint32_t i = 0; i < FETCH_WIDTH; i++) {
    ...
    if (ROB.entry[read_index].translated) {
      if (read_index == ROB.head)
        ...
        cout << "read_index: " << read_index << " ROB.head: " << ROB.head
             << " ROB.tail: " << ROB.tail << endl;
        assert(0);
      }
    }
#endif
  ...
  uint32_t fetch_index = (ROB.last_fetch == (ROB.SIZE - 1))
                           ? 0
                           : (ROB.last_fetch + 1);
  ...
  for (uint32_t i = 0; i < FETCH_WIDTH; i++) {
    ...
    if ((ROB.entry[fetch_index].translated != COMPLETED) ||
        (ROB.entry[fetch_index].event_cycle > current_core_cycle[cpu]))
      break;
    ...
    if (ROB.entry[fetch_index].fetched) {
      if (fetch_index == ROB.head)
        ...
        cout << "fetch_index: " << fetch_index << " ROB.head: " << ROB.head
             << " ROB.tail: " << ROB.tail << endl;
        assert(0);
      }
    }
...
void O3_CPU::reg_RAW_dependency(uint32_t prior,
                                uint32_t current,
...
void O3_CPU::mem_RAW_dependency(uint32_t prior,
                                uint32_t current,
                                uint32_t data_index,
                                uint32_t lq_index)
{
...
void O3_CPU::operate_lsq()
{
    ...
    if (RTS0[RTS0_head] < SQ_SIZE) {
      ...
      if (SQ.entry[sq_index].event_cycle <= current_core_cycle[cpu]) {
        ...
        if (knob::knob_cloudsuite)
          ...
          data_packet.address = SQ.entry[sq_index].virtual_address >>
                                LOG2_PAGE_SIZE;
  ...
  while (store_issued < SQ_WIDTH) {
    if (RTS1[RTS1_head] < SQ_SIZE) {
      ...
      if (SQ.entry[sq_index].event_cycle <= current_core_cycle[cpu]) {
        execute_store(SQ.entry[sq_index].rob_index,
                      sq_index,
    ...
    if (RTL0[RTL0_head] < LQ_SIZE) {
      ...
      if (LQ.entry[lq_index].event_cycle <= current_core_cycle[cpu]) {
        ...
        if (knob::knob_cloudsuite)
          ...
          data_packet.address = LQ.entry[lq_index].virtual_address >>
                                LOG2_PAGE_SIZE;
  ...
  while (load_issued < LQ_WIDTH) {
    if (RTL1[RTL1_head] < LQ_SIZE) {
      ...
      if (LQ.entry[lq_index].event_cycle <= current_core_cycle[cpu]) {
        int rq_index = execute_load(LQ.entry[lq_index].rob_index,
                                    lq_index,
...
void O3_CPU::execute_store(uint32_t rob_index,
                           uint32_t sq_index,
                           uint32_t data_index)
{
  ...
  if (ROB.entry[rob_index].is_producer) {
    ITERATE_SET(dependent,
                ROB.entry[rob_index].memory_instrs_depend_on_me,
                ROB_SIZE)
    {
...
int O3_CPU::execute_load(uint32_t rob_index,
                         uint32_t lq_index,
                         uint32_t data_index)
{
  // add it to L1D
  ...
  // keep track of data footprint
  unique_ppages.insert(LQ.entry[lq_index].physical_address >> LOG2_PAGE_SIZE);
